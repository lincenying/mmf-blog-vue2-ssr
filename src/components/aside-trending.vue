<template>
    <div class="card card-trending">
        ...
        <div class="card-content">
            <div v-for="(item, index) in trending.list" class="trending-item">
                <span class="trending-rank-num">{{ index + 1 }}</span>
                <router-link :to="`/article/${item._id}`" class="trending-title">{{ item.title }}</router-link>
                <div class="trending-meta">
                    <div class="trending-meta-item"><i class="icon icon-action-voteup"></i>{{ like }}</div>
                    <div class="trending-meta-item"><i class="icon icon-action-comment"></i>{{ comment_count }}</div>
                </div>
            </div>
        </div>
    </div>
